...
function inputFocus(e){
   document.querySelector(`label[for=${e.target.id}]`).classList.add("hero__label--active");   
}

function inputBlur(e){
   if(!e.target.value){
      document.querySelector(`label[for=${e.target.id}]`).classList.remove("hero__label--active");
   }
}
